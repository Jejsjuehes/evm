...
const { loadData } = require("./src/utils");
...
async function retry(fn, maxRetries = MAX_RETRIES, delay = RETRY_DELAY) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      console.log(colors.yellow(`⚠️ Error occurred: ${error.message} | Retrying... (${i + 1}/${maxRetries})`));
      await sleep(delay);
    }
  }
}
...
const main = async () => {
  ...
  let privateKeys = loadData("privateKeys.txt");
  privateKeys = privateKeys.map((key) => (key.startsWith("0x") ? key : `0x${key}`));

  const recipientAddresses = loadData("wallets.txt");

  const transactionCount = config.number_transactions;
  // readlineSync.questionInt("Enter the number of transactions you want to send for each address: ");
  ...
  for (const privateKey of privateKeys) {
    ...
    if (senderBalance < ethers.parseUnits("0.00001", "ether")) {
      console.log(colors.red("❌  Insufficient balance under 0.00001 ETH. Skipping to next address."));
      continue;
    }
    ...
    const printSenderBalance = async () => {
      while (continuePrintingBalance) {
        try {
          senderBalance = await retry(() => checkBalance(provider, senderAddress));
          console.log(colors.blue(`💰 Current Balance: ${ethers.formatUnits(senderBalance, "ether")} ${selectedChain.symbol}`));
          if (senderBalance < ethers.parseUnits("0.00001", "ether")) {
            console.log(colors.red("❌ Insufficient balance for transactions. Under 0.00001 ETH."));
    ...
    for (let i = 1; i <= transactionCount; i++) {
      for (const receiverAddress of recipientAddresses) {
        ...
        const amountToSend = ethers.parseEther((Math.random() * (config.amount[1] - config.amount[0]) + config.amount[0]).toFixed(6));
        ...
        console.log(colors.white(`  Amount: ${colors.green(ethers.formatUnits(amountToSend, "ether"))} ${selectedChain.symbol}`));
        ...
        try {
          ...
          if (receipt) {
            if (receipt.status === 1) {
              ...
              console.log(colors.green(`  Transaction hash: ${selectedChain.explorer}/tx/${receipt.hash}`));
            } else {
  ...
  console.log(colors.yellow("Tool được phát triển bởi nhóm telegram: https://t.me/airdrophuntersieutoc"));
  process.exit(0);
};
